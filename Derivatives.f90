...
!$OMP DO
  do j =2,Jmax-1
    do i =2,Imax-1
      ...
      +(E2_vis_a_right(i,j)+E2_vis_a_left(i,j))/ddx(i-1)      &
      +(F2_vis_a_upper(i,j)+F2_vis_a_lower(i,j))/ddy(j-1)
      ...
      +(E2_vis_b_right(i,j) + E2_vis_b_left(i,j)) /ddx(i)       &
      +(F2_vis_b_upper(i,j) + F2_vis_b_lower(i,j))/ddy(j-1)
      ...
      +(E2_vis_c_right(i,j)+E2_vis_c_left(i,j))/ddx(i) &
      +(F2_vis_c_upper(i,j)+F2_vis_c_lower(i,j))/ddy(j)
      ...
      +(E2_vis_d_right(i,j)+E2_vis_d_left(i,j))/ddx(i-1) &
      +(F2_vis_d_upper(i,j)+F2_vis_d_lower(i,j))/ddy(j)
